...
- Spatial extraction:
  ...
  - `span_m`: averaging half-width around the stretcher center, in **m** for each side (±`span_m`).
- Trace units:
...
## Frequency Response Engine (calculate_frequency_response)
...
- Steps:
  ...
  - Define frequency window (Nyquist fractions):
...
- The method assumes frequency-stepped excitation over the selected analysis band and adequate SNR at each step.
- `fs` must be high enough for the chosen Nyquist-fraction analysis window and to avoid aliasing.
- The spatial window `[center_m - span_m, center_m + span_m]` must be correctly positioned around the stretcher region.
